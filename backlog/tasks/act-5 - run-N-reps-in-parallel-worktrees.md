...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 CLI tests observe that existing replay and benchmark commands still run one debug rep when confirmation is absent; --confirm selects confirmation with 5 reps, --confirm --reps N overrides N, --reps without --confirm and confirmation counts below 2 are rejected, and debug output says "single-rep evidence, not a score" while retaining the individual Judge result.
- [x] #2 A coordinator test blocks three fake reps at a barrier and observes all three started before any is released; each receives a distinct rep ID and worktree path while sharing byte-identical frozen corpus, rubric, model, effort, source SHA or checkpoint lineage, and pipeline inputs.
- [x] #3 Integration tests observe both three stage replays and three full-pipeline reps execute in detached worktrees without changing the primary checkout HEAD, branch, status, or files; stages remain sequential inside each full-pipeline rep while reps overlap in time.
- [x] #4 Before a confirmation creates a worktree or starts a session, CLI output shows a deterministic maximum cost derived from configured budgets and maximum worker, PO, and Judge calls including retries; execution waits for explicit approval, and --yes supplies that approval noninteractively.
- [x] #5 Every rep writes a collision-free strict record containing its ordinal, outcome, stage evidence, lineage, actual role-attributed cost and token usage, worker trajectory steps, monotonic elapsed durations, and retained result evidence; the group writes its frozen inputs, projected cost and approval, rep-record references, and aggregate report.
- [x] #6 For every declared stage and the final outcome, a 5-rep report shows attempted, not-reached, failed, and successful counts; the raw Judge-grade distribution; binary success rate p with CLT standard error sqrt(p(1-p)/n); and pass^k = p^k with k equal to the requested rep count. It also shows per-role and total cost and input/output/cache token distributions, worker-turn distributions, per-stage elapsed-duration distributions, and overall parallel makespan.
- [x] #7 A declared stage outcome is successful only for Judge grade A or B; the final outcome is successful only for final Judge PASS. STOP, final FAIL, execution failure, and missing required provider metrics are unsuccessful and are never converted to zero-valued evidence.
- [x] #8 When one rep fails or stops, the other reps finish; that rep records the failure and preserves its worktree when diagnosis requires it, later stages are marked not reached, aggregate denominators expose attempted and not-reached reps, and full-pipeline reliability counts the rep as unsuccessful.
- [x] #9 After every completed rep, its worktree is removed even when its Judge rejects the result; after an induced pre-evidence failure, the preserved worktree path is printed. Concurrent worktree creation, record writes, and cleanup do not collide.
<!-- AC:END -->
...
Build completion 2026-09-01, full-pipeline confirmation: run-benchmark.ts keeps debug execution at one labeled rep and routes explicit confirmation through deterministic approval before source access. Approved runs freeze one task/checkpoint and all controlled inputs, overlap isolated detached reps while preserving sequential stages, write strict rep/group/report evidence, retain cited result SHAs, clean completed worktrees, preserve and print only diagnostic failures, and leave the primary checkout unchanged. Late stage and final-Judge failures retain their exact completed stage evidence, failure locus, result SHA, per-rep elapsed time, and available role metrics. Missing metrics on any attempted provider call make the rep and every reliability summary ineligible for success without erasing raw grades. STOP does not require a call that was correctly not reached. A production subprocess test observes approval before target access without invoking a provider. Full verification: 291 tests pass, 0 fail; typecheck, lint, and format check pass. No Claude or paid execution ran.

Independent review 2026-09-01: five blocking findings, five should-fix findings, and one note; no security findings. Fixed all five blockers: late failure context, metric-ineligible reliability, STOP metric completeness, STOP retention, and worker-boundary timing. Fixed three should-fix findings: completed-rejection temporary-root cleanup, production CLI routing coverage, and collision-sensitive final evidence coverage. Tracked test organization and shared confirmation harnesses in ACT-13. Tracked replay/pipeline evidence-lifecycle duplication in ACT-21. Fixed the style note by declaring the Judge input type directly. Every finding is disposed; no decision remains open.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Implemented explicit N-rep confirmation for both stage replay and full pipelines. Confirmation now freezes controlled inputs once, runs isolated detached reps concurrently, records strict reliability/resource evidence, gates projected cost before execution, preserves exact failure context, and cleans or retains worktrees according to diagnostic need. Verified with 291 passing tests plus typecheck, lint, and formatting; no paid execution was used.
