...
What landed: an append-only run-event store in SQLite, derived and rebuildable per decision-3, with the records on disk still authoritative; events at a stage's start, each turn's completion, each stage's completion, and every terminal outcome; an SSE endpoint serving one run's events; a startup reconciliation pass that checks whether a run's process is still alive before declaring it interrupted; and INTERRUPTED reaching every reader that renders run status. Stage-progress `console.log` sites that this card's own scope touched are gone (AC #4); ACT-26.7 itself, the 17-site stdout/stderr split for `run --json`/`replay --json`, is not closed by this card and remains 0/6 (verified via `reflection: ACT-51`, doc-42: 11 console.log sites remain in src/benchmark/).
